...
class DARCalculator:
    ...
    def _calcDARHelperX(self, press, temp, targWlen, refWlen, zangles):
        """
        press in hPA
        temp in degC
        ...
        """
        refa, refb = self._calcRefParsX(press, temp, refWlen)
    ...
    def _calcDARHelper(self, press, temp, targWlen, refWlen, zangles):
        ...
        """
        refa, refb = self._calcRefParsX(press, temp, refWlen)
        targRefa, targRefb = self._calcRefParsX(press, temp, targWlen)

        refAR = self._calcAR(refa, refb, zangles)
        targAR = self._calcAR(targRefa, targRefb, zangles)
        print('DARH',refAR - targAR)
        return (refAR - targAR)
    ...
    def calcDAR (self, haDeg, targetWavelength, tlist):
        ...
        dars = self._calcDARHelperX(self.pressureMMHg, self.temperature, targetWavelength, self.centerWavelength, zenithDistRad)
    ...
    def getRefr(self, raDegs, decDegs, raDeg0, haDeg):
        ...
        """
        refa, refb = self._calcRefParsX(
            self.pressureMMHg, self.temperature, self.centerWavelength
        )
        print('get refab',refa,refb)
        print(self.pressureMMHg, self.temperature, self.centerWavelength)
#        import sl
#        print(sl.slrfco(hm,tdk,pmb,rh,wl,phi,tlr,eps))
    ...
    def getRefTest(self, raDegs, decDegs, raDeg0, haDeg):
        """
        Returns refractetd raDegs, decDegs, and refracted amount in degress
        """
        refa, refb = self._calcRefParsX(self.pressureMMHg, self.temperature, self.centerWavelength) #461.287934863083, 0, 750.0
        print('get refab',refa,refb)
        hm,tdk,pmb,rh,wl,phi,tlr,eps=4150,273.15,461.287934863083,0.4,.750,0.3455752,0.0065,1e-10
        import sl
        ra,rb=sl.slrfco(hm,tdk,pmb,rh,wl,phi,tlr,eps)
        ...
        zd=sl.slrefz(zu,ra,rb)
        return refa,refb,ra,rb,self.pressureMMHg, self.temperature, self.centerWavelength,zu-zd,el


    def getUnRefr(self, raDegs, decDegs, raDeg0, haDeg):
        """
        Returns refractetd raDegs, decDegs, and refracted amount in degress
        """
        refa, refb = self._calcRefParsX(
