...
#        print('Slits1')
       # print(slits1.to_csv(None))
#        print(slits1.OBJECT.to_string())
#        print('Slits2')
#        print(slits2.OBJECT.to_string())
       # print(slits2.to_csv(None))
    ...
    def plotSlits (self, annotate=False):
        ...
        drawUtils.drawPatch(ax, codes1, fc="None", ec="r", alpha=0.5, label=os.path.basename(self.fitsname1)[-10:])        
        drawUtils.drawPatch(ax, codes2, fc="None", ec="b", alpha=0.5, label=os.path.basename(self.fitsname2)[-10:])
        drawUtils.drawPatch(ax, layoutMM, fc="None", ec="g")

        plt.plot (allSlits.xs1, allSlits.ys1, 'r.',alpha=0.5) 
        print(allSlits.xs1, allSlits.ys1)
        if annotate:
            drawUtils.annotate(ax, allSlits.OBJECT, allSlits.xs1, allSlits.ys1, color='r',alpha=0.5)
            drawUtils.annotate(ax, allSlits.OBJECT, allSlits.xs2, allSlits.ys2, color='b',alpha=0.5)
        
        plt.plot (allSlits.xs2, allSlits.ys2, 'b.',alpha=0.5)
        print(allSlits.xs2, allSlits.ys2)

#        codes1c = self.calcSlitXYjoined(allSlits,"_x")
#        print(codes1c)
#        drawUtils.drawPatch(ax, codes1c, fc="None", ec="black", alpha=0.9,label=os.path.basename(self.fitsname1)[-10:])
    ...
    def calcSlitXYjoined (self, slits,suffix):
        """
        Calculates the slit corners using slit ceneters in RA/DEC and lengths
        """
        slitRADeg = slits[f"slitRA{suffix}"]
        slitDec = slits[f"slitDec{suffix}"]

        slitAngle = np.radians(slits[f"slitLPA{suffix}"]-90)
        half = slits[f"slitLen{suffix}"] / 2 / 3600
        cosdec = np.cos (np.radians(slitDec))
        ...
        upX = sines * slits[f"slitWid{suffix}"]/2 / 3600
        upY = cosines * slits[f"slitWid{suffix}"]/2 / 3600
